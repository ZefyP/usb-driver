...
#define RS_ATTR_OPC_TIMEOUT                                 /* ViInt32,   hidden */             (RS_ENGINE_PUBLIC_ATTR_BASE + 1001) // OPC timeout attribute
...
// RsCoreRangeTableDiscrete contains an array of RsCoreRangeTableDiscrete structures - pairs of cmdNumericValue + cmdString
typedef struct RsCoreRangeTableDiscreteEntry
{
    ViInt32     cmdNumericValue;
    ViString     cmdString;
} RsCoreRangeTableDiscreteEntry, *RsCoreRangeTableDiscreteEntryPtr;

// Discrete range table, containign Discrete RangeTable Entries
typedef struct RsCoreRangeTableDiscrete
{
    ViInt32                entriesCount;
    RsCoreRangeTableDiscreteEntryPtr entries;
} RsCoreRangeTableDiscrete, * RsCoreRangeTableDiscretePtr;

typedef struct RsCoreRangeTableRanged
{
    ViReal64 min;
    ViReal64 max;
    ViString unit;
} RsCoreRangeTableRanged, *RsCoreRangeTableRangedPtr;

typedef struct RsCoreRangeTableCoerced
{
    ViInt32 entriesCount;
    ViAReal64 entries;
    ViString unit;
} RsCoreRangeTableCoerced, *RsCoreRangeTableCoercedPtr;
...
#define RS_VAL_WAIT_FOR_OPC      (1L << 0)   // Specifies whether the attribute is OPC-synchronised
#define RS_VAL_DONT_CHECK_STATUS (1L << 1)   //  If set, the driver skips checking of the instrument status after performing the attribute command
...
/******************************************************************************************************
 ...
 FALSE causes:
- Skipping checking of instrument model and options
...
typedef ViStatus (*RsCoreErrorMessage)(ViSession, ViStatus, ViChar*);
...
typedef struct RsCoreAttribute
{
    ...
    RsCoreRangeTableDiscretePtr   rangeTableDiscrete;
    RsCoreRangeTableRangedPtr     rangeTableRanged;
    RsCoreRangeTableCoercedPtr    rangeTableCoerced;
    ViString                      command;
    ViInt32                       int32Dflt;
    ViInt64                       int64Dflt;
    ViReal64                      doubleDflt;
    ViBoolean                     booleanDflt;
    ViString                      stringDflt;
    RsCoreAttrFlags               flags;
    ViBoolean                     hasRepCaps;
    ViString                      supportedModels;
    ViString                      instrOptions;
...
typedef struct RsCoreSession
{
	...
	ViChar                    idnString[RS_MAX_MESSAGE_LEN]; // Idn string of the instrument. In case of simulation, the value is equal to the PREFIX_SIMULATION_ID_QUERY
    ...
    ViChar                    scpiQueryParam[RS_MAX_MESSAGE_LEN]; // SCPI parameter string transferred by the GetAttribute function to the core function RsCoreInt_QueryAttrCommandUni()
    ...
    ViBoolean                 smartEnumMatching; // If TRUE (default is TRUE), SCPI string instrument responses are matched to range table items in a smart way
	...
	ViBoolean				  useExternalSession; // If TRUE (default is FALSE), the driver used an externally opened Visa Session, and will not close it in the RsCore_ViClose
	ViBoolean				  saveLastComposedCmd; // If True (default is FALSE), you get the last composed SCPI command in the field lastComposedCmd
	ViChar                    lastComposedCmd[RS_MAX_MESSAGE_BUF_SIZE]; // If the saveLastComposedCmd is VI_TRUE, this field contains the last SCPI command sent to the instrument
	ViBoolean                 hasK0; // If TRUE, the optionList contains K0 entry, meaning all sw options are available
	ViBoolean                 allowK0override; // If TRUE (default is TRUE), and the 'hasK0' is TRUE, the option checking functions return TRUE. If FALSE, the 'hasK0' is ignored
...
void RsCore_AfterErrorInInit(ViSession        instrSession,
                             RsCoreSessionPtr sessionProperties,
                             ViStatus         error,
                             ViPSession       newInstrSession);
...
// ----------------------- Get Attributes with query parameters----------------------------------
